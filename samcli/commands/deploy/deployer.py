...
class Deployer(object):
    ...
    def desribe_stack_events(self, stack_name):
        ...
        while loop:
            resp1 = self._client.describe_stacks(StackName=stack_name)
            if 'IN_PROGRESS' not in resp1['Stacks'][0]['StackStatus']:
                loop = False
            ...
            for event in resp['StackEvents']:
                if event['EventId'] not in events and event['Timestamp'] > utc_now:
                    ...
                    click.secho(message="\t" + event['LogicalResourceId'] + "\t" + event['ResourceType'] + "\t" + event['ResourceStatus'], fg="green")
        # return self._client.execute_change_set(
        #     ChangeSetName=changeset_id,
        #     StackName=stack_name)
